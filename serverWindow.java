...
public class serverWindow {
	...
	private static String ip_server = "169.255.1.1";
	...
	private static Map<String, Socket> allUsersListIP = new ConcurrentHashMap<>();// usernames used and their socket connections
	private static Set<String> activeUserSet = new HashSet<>(); // this set keeps track of all the active users 
	private static Set<String> activeUserSetIP = new HashSet<>();
	...
	private DefaultListModel<String> activeCListIP = new DefaultListModel<String>();
		...
		public void run() {
			while (true) {
				try {
					...
					String ipClient = new DataInputStream(clientSocket.getInputStream()).readUTF(); //new
					...
					if (activeUserSet != null && activeUserSet.contains(idClient)) { // if username is in use then we need to prompt user to enter new name
						...
						if (activeUserSet != null && activeUserSet.isEmpty()) { 
							//parte in cui diventa coordinatore
							coordinator = idClient;
							coordinatorip = ipClient;
							cOutStream.writeUTF("coordinator");
						}
						...
						activeCListIP.addElement(ipClient); //new
						...
						new MsgRead(clientSocket, idClient, ipClient).start(); // create a thread to read messages
	...
	class MsgRead extends Thread { // this class reads the messages coming from client and take appropriate actions
		...
		String Id;
		String Ip;
		private MsgRead(Socket socket, String idClient, String ipClient) { // socket and id will be provided by client
			this.socket = socket;
			this.Id = idClient;
			this.Ip = ipClient;
		}
				...
				try {
					...
					if (msgList[0].equalsIgnoreCase("multicast")) { // if action is multicast then send messages to selected active users
						...
						while (itr.hasNext() && itr1.hasNext()) {
							...
							if (!idClient2.equalsIgnoreCase(Id) && !ipClient2.equalsIgnoreCase(Ip)) { // we don't need to send this message to ourself
								try {
									if (activeCList.get(0).equals(Id)) { //if the first client (coordinator) left, the next one will be the coordinator
										...
										//coordinatorip = activeCListIP.get(1);
										new DataOutputStream(((Socket) allUsersList.get(idClient2)).getOutputStream())
										.writeUTF(Id + " disconnected..." + "\n" + "The new Coordinator is: " + coordinator); // notify all other active user for disconnection of a user and the new coordinator id 
						...
						activeCList.removeElement(Ip); // remove client from Jlist for server
	...
	class PrepareCLientList extends Thread { // it prepares the list of active user to be displayed on the UI
		...
		public void run() {
			try {
				...
				while (itr.hasNext() && itr1.hasNext()) { // prepare string of all the users
				...
				if (ids.length() != 0) { // just trimming the list for the safe side.
					...
					ips = ips.substring(0, ips.length() - 1);	//new
				}
				itr = activeUserSet.iterator(); 
				itr1 = activeUserSetIP.iterator();
				while (itr.hasNext() && itr1.hasNext()) { // iterate over all active users
					String key = (String) itr.next();
					String key1 = (String) itr1.next();
					try {
						new DataOutputStream(((Socket) allUsersListIP.get(key1)).getOutputStream())
						.writeUTF("----->" + ips);	
