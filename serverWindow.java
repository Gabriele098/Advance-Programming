			...
			while (true) {
				try {
					...
					if (activeUserSet != null && activeUserSet.contains(uName)) { // if username is in use then we need to prompt user to enter new name
						...
						OnlineClientsServerList.setModel(activeDlm); // show the active and allUser List to the swing app in JList
						AllClientsServerList.setModel(allDlm);
						txtrDisplayServer.append("Client " + uName + " Connected...\n"); // print message on server that new client has been connected.
			...
			while (AllClientsServerList != null && !allUsersList.isEmpty()) {  // if allUserList is not empty then proceed further
				try {
					...
					if (msgList[0].equalsIgnoreCase("multicast")) { // if action is multicast then send messages to selected active users
						...
						OnlineClientsServerList.setModel(activeDlm); //update the active user list
					}
				} catch (Exception e) {
